...
df_modelos <- tibble(
  ...
  mutate(
    formula = glue::glue(
      "log(share) ~ carac1 | firma + mercado | ",
      "preco ~ {instrumentos}"),
    modelo = map(formula, ~feols(as.formula(.x), data = df_dados_iv)),
    parametros = map(modelo, broom::tidy),
    elasticidade = map(parametros, "estimate") %>% 
      map_dbl(1),
    medidas = map(modelo, broom::glance)
  )

# etable(df_modelos$modelo, stage = 1:2)
ggplot(df_modelos, aes(fct_reorder(instrumento, elasticidade),
                       elasticidade)) + 
  geom_col(width = 0.1) + 
  geom_point() + 
  coord_flip() + 
  theme_minimal() + 
  labs(x = "Instrumento", y = "Elasticidade")
```
...
Tomando o modelo que usa instrumentos de Hausman (modelo 13), temos
$\alpha$ = -1.682 e $\beta$ = 0.5168.
